...
imputed_dataset = imputed_dataset %>% dplyr::select(PRACTICE_PATIENT_ID,age,male,eGFR,acr,time,status,ethnicity,diabetic)
...
model_info <- list(
  "coefficients" = stats::coef(model1),
  "baseline_hazards" = lapply(model1$models, function(mod) survival::basehaz(mod, centered = FALSE)),
  "model_terms" = lapply(model1$models, function(mod) mod[["terms"]])
)

# Can be saved to then be shared as:
saveRDS(model_info, file = "model_info_updated_kfre_only.rds")

saveRDS(model1, "model_full_updated_kfre_only.rds")
...
# Can be saved to then be shared as:
saveRDS(model_info, file = "model_info_updated_kfre_plus.rds")

saveRDS(model1, "model_full_updated_kfre_plus.rds")
