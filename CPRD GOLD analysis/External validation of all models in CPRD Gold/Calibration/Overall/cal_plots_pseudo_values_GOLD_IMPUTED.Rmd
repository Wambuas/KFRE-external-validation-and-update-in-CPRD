...
#### Calculating the risk scores using the KFREUK1 model - using saved model infomation file
...
predictRisk_shared_CSC <- function(model_info, newdata, horizon, primary_event) {
  ...
  mm_rhs <- function(terms_obj, data) {
    tt <- stats::delete.response(terms_obj)
  ...
  # ---- linear predictors for all causes
...
kfre_uk1$status = kfre_uk1$status1
kfre_uk1$time = kfre_uk1$time1


kfre_uk1$score_2yrs_KFREUK1_v2 <- predictRisk_shared_CSC(
  kfre_only_model,
  newdata = kfre_uk1,
  horizon = horiz, 
...
kfre_uk1$score_5yrs_KFREUK1_v2 <- predictRisk_shared_CSC(
  kfre_only_model,
  newdata = kfre_uk1,
  horizon = horiz, 
  primary_event = cause_interest)
...
```{r}
all.equal(kfre_uk1$score_2yrs_KFREUK1, kfre_uk1$score_2yrs_KFREUK1_v2)
```


```{r}
all.equal(kfre_uk1$score_5yrs_KFREUK1, kfre_uk1$score_5yrs_KFREUK1_v2)
```
